...
function addChocolates(chocolates, color, count){
  return count<=0
  ?"Number cannot be zero/negative": helperAdd(chocolates, color, count);
}
...
function removeChocolates (chocolates, count){
  ...
  return count <=0?"Number cannot be zero/negative":
  chocoStore<count ? "Insufficient chocolates in the dispenser"
...
function dispenseChocolates(chocolates,number){
  const chocoStore = chocolates.length;
  return number<=0? "Number cannot be zero/negative":chocoStore<number?"Insufficient chocolates in the dispenser" : helperDispense(chocolates, number)
}
...
function changeChocolateColor(chocolates, number, color, finalColor){
  return number<=0 ? "Number cannot be zero/negative": color==finalColor ? "Can't replace the same chocolates":helperToChangeChocolateColor(chocolates, number, color, finalColor);
}
